...
#context {
  assert.eq(
    query(<m-a>).len(),
    1,
    message: "handout: unlabelled section must render",
  )
  assert.eq(
    query(<m-b>).len(),
    0,
    message: "handout: <touying:never> section must not render",
  )
  assert.eq(
    query(<m-c>).len(),
    1,
    message: "handout: never-presentation is not a keyword and must filter nothing",
  )
  assert.eq(
    query(<m-d>).len(),
    1,
    message: "handout: unlabelled slide must render",
  )
  assert.eq(
    query(<m-e>).len(),
    0,
    message: "handout: <touying:never> slide must not render",
  )
}
