...
///
/// - show-notes (boolean): Whether to show the speaker notes as the main content with the slide shown as a small thumbnail in the top right corner. Default is `false`.
///
...
#let _default-show-notes(self: none, width: 0pt, height: 0pt, cutout: false) = {
...
///
/// - show-notes (function): The function to show notes on second screen. It should be `(self: none, width: 0pt, height: 0pt, cutout: false) => { .. }` with core code `utils.current-slide-note` and `utils.slide-note-state.update(none)`.
///
///   When `cutout: true` (used by `config-common(show-notes: true)` mode), the function should return a dictionary with `background`, `foreground`, and `cutout-height` keys.
///
