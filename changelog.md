...
- **feat: article mode, one source file for slides or a flowing article**
  ...
  Two markers let you write for the article target. `#article-text[..]` replaces the content of the slide it is written in, wherever in that slide you write it, so you can give a prose paragraph instead of bullet points. `#article-only[..]` adds content that appears only in the article (for more see mode-only content below). The new `config-article(..)` group covers article-side layout: `wrap-images`, `wrap-image-figures`, `wrap-other-figures`, `wrap-other`, `wrap-width` and `wrap-align-direction` for side-floating (done with `meander`), `linearize` for how much of the deck's layout comes apart (a `#columns(..)`, a composer and a `side-by-side` always do; a `table` or `grid` only until it declares a header or footer), plus an optional `title-block-fn`, and `available-fields`: a mapping that hands configuration values such as `info.title` through to the article theme's own parameters.
